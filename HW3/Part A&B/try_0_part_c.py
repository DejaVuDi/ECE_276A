...
# (c) Visual-Inertial SLAM (Extra Credit)
	...
	# (b) Landmark Mapping via EKF Update & (c) Visual-Inertial SLAM (Extra Credit)
		...
		for j in range(num_land):
			if np.logical_and(((z[:,j] != z_0).all()),((temp[:,j] == z_0).all())):
				...
				landmark_world = inv(mu)@inv(cam_T_imu)@invM(temp[:,count],f_su,c_u,f_sv,c_v,b)#landmark_img
	...
	# (c) Visual-Inertial SLAM (Extra Credit)	
		...
		z_hat_1 = M@(slam_q/slam_q[2,:])
		dot = odot(slam)
		for o in range(num_land):
			if (map_mu[:,o] == z_0).all():
				slam_H[(4*o):(4*o+4),:] = np.zeros((4,6))
			else:
				another = mu@map_mu[:,o]
				...
				slam_d_q = (1/one[2])*np.array([[1,0,-one[0]/one[2],0],
												...
												[0,0,0,0],
												[0,0,-one[3]/one[2],1]])
				slam_H[(4*o):(4*o+4),:] = M@slam_d_q@cam_T_imu@dot[:,:,o]
		slam_K = sigma@slam_H.T@pinv(slam_H@sigma@slam_H.T+slam_V)
		mask = np.all((z_1+1)!=0,axis=0)
		mu = expm(hat(slam_K@((((z_1-z_hat_1)*mask).swapaxes(0,1)).reshape((4*num_land,1)))))@mu
		# mu = expm(hat(slam_K@(((z_1-z_hat_1).swapaxes(0,1)).reshape((4*num_land,1)))))@mu
		sigma = (np.identity(6)-slam_K@slam_H)@sigma 
...
# slam_q = cam_T_imu@slam
# z_hat_1 = M@(slam_q/slam_q[2])
# slam_d_q = (1/slam_q[2])*np.array([[1,0,-slam_q[0]/slam_q[2],0],
# 								[0,1,-slam_q[1]/slam_q[2],0],
# 								[0,0,0,0],
# 								[0,0,-slam_q[3]/slam_q[2],1]])
# dot = np.array([[slam[3], 0,       0,       0,        slam[2], -slam[1]],
# 				[0,       slam[3], 0,      -slam[2],  0,       -slam[0]],
# 				[0,       0,       slam[3], slam[1], -slam[0],        0],
# 				[0,       0,       0,       0,        0,              0]])
# slam_H = M@slam_d_q@cam_T_imu@dot
# slam_K = sigma@slam_H.T@pinv(slam_H@sigma@slam_H.T+slam_V)
# mu = expm(hat(slam_K@(z_1[:,j]-z_hat_1)))@mu
# sigma = (np.identity(6)-slam_K@slam_H)@sigma