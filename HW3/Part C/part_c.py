...
# (a) IMU Localization via EKF Prediction
	...
	T_ = np.zeros((4,4,t.size-1))
	mu = np.identity(4) # for IMU
	mu_old  = np.identity(4)
...
# (c) Visual-Inertial SLAM (Extra Credit)
	...
	slam_V = 10*np.identity(4*num_land)
	...
	# (c) Visual-Inertial SLAM (Extra Credit)	
		...
		map_mu = map_mu + (slam_D@K[0:3*num_land,:]).reshape((num_land,4)).T
		...
		mu_old = expm(-tau[0]*u_hat)@mu_old
		P_ = inv(mu_old)
		T_[:,:,i] = P_
	...
	visualize_trajectory_2d(T_,T,map_mu,path_name=data)